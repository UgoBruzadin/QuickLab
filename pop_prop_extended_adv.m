function [fh, EEG, com] = pop_prop_extended_adv(EEG, typecomp, chanorcomp, winhandle, spec_opt, erp_opt, scroll_event, classifier_name, varargin)
...
if isobject(winhandle) || ~isnan(winhandle)
    ...
    % 
plotTFcommand = ['figure; pop_newtimef( EEG, 0, ' num2str(chanorcomp) ' , [' num2str(EEG.xmin*1000) ' ' num2str(EEG.xmax*1000) '], [3 0.8] , ''topovec'', EEG.icawinv(:,', int2str(chanorcomp), '), ''elocs'', EEG.chanlocs, ''chaninfo'', EEG.chaninfo, ''caption'', ''[ ' strcat('IC ',int2str(chanorcomp))  ']'', ''baseline'',[0], ''plotphase'' , ''off'', ''padratio'', 1); '];
...
% 	h  = uicontrol(gcf, 'Style', 'pushbutton', 'string', 'OK', 'backgroundcolor', GUIBUTTONCOLOR, 'Units','Normalized', 'Position',[0.85 bottom 0.1 height], 'callback', command);
...
% 	end;
	...
	com = sprintf('pop_prop_extended_adv( %s, %d, [%s], 0, %s, %s, %d, ''%s''', inputname(1), ...
                  typecomp, int2str(chanorcomp), vararg2str({spec_opt}), vararg2str({erp_opt}), scroll_event, classifier_name);
    if ~isempty(varargin)
        com = [com sprintf(', %s', vararg2str(varargin))];
    end
